...
export const PawariCulturalSection: React.FC<PawariCulturalSectionProps> = ({ initialTab = 'shabdkosh' }) => {
  ...
  const approvedShabdkosh = (shabdkoshList || []).filter(s => s.status === 'approved' || s.status === 'published' || s.status === 'active' || (!s.status && !s.id.startsWith('contrib_')));
  const approvedPaheli = (paheliList || []).filter(p => p.status === 'approved' || p.status === 'published' || p.status === 'active' || (!p.status && !p.id.startsWith('contrib_')));
